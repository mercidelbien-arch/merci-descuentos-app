...
app.get("/admin/campaigns", (req, res) => {
  ...
  return res.end(
    ...
    "<style>body{font-family:system-ui;margin:0;background:#f7f8fa;color:#0f172a}" +
    ...
    ".main{padding:24px}.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:16px}" +
    ".card{background:#fff;border:1px solid #e5e7eb;border-radius:14px;box-shadow:0 1px 3px rgba(0,0,0,.04);padding:18px;text-decoration:none;color:inherit;display:block;transition:.15s}" +
    ...
    ".title{font-weight:700;margin:0 0 4px}.muted{color:#64748b;font-size:14px;margin:0}</style></head>" +
    "<body><div class='layout'>" +
    "  <aside class='aside'><div class='brand'>Merci Descuentos</div>" +
    "    <nav class='nav'>" +
    ...
    "    </nav>" +
    "  </aside>" +
    "  <main class='main'><h1 style='margin:0 0 14px'>Campañas</h1>" +
    "    <div class='grid'>" +
    "      <a class='card' href='/admin/?store_id=" + store_id + "'>" +
    "        <div class='icon'>%</div><h3 class='title'>Cupones %</h3>" +
    "        <p class='muted'>Descuento en subtotal del carrito usando código.</p>" +
    "      </a>" +
    "      <div class='card' onclick='alert(\"Próximo: 3x2\")'><div class='icon'></div><h3 class='title'>3×2</h3><p class='muted'>Lleva X, pagá Y (próximo).</p></div>" +
    "      <div class='card' onclick='alert(\"Próximo: progresivo\")'><div class='icon'></div><h3 class='title'>Progresivo</h3><p class='muted'>Descuento por cantidad (próximo).</p></div>" +
    "    </div>" +
    "  </main></div></body></html>"
  );
});
...
app.get("/admin", async (req, res) => {
  ...
  return res.end(
    "<!doctype html><html lang='es'><head><meta charset='utf-8'/>" +
    "<title>Cupones Merci</title><meta name='viewport' content='width=device-width, initial-scale=1'/>" +
    "<style>body{font-family:system-ui;padding:24px;max-width:980px;margin:auto;background:#fafafa;color:#111}" +
    "h1{margin:0 0 8px}.card{background:#fff;border:1px solid #eee;border-radius:12px;box-shadow:0 1px 3px rgba(0,0,0,.04);padding:16px;margin:16px 0}" +
    "label{display:block;font-size:12px;color:#555;margin-top:10px}input,select,textarea{width:100%;padding:8px 10px;border:1px solid #ddd;border-radius:8px;margin-top:6px}" +
    ...
    "button{background:#111;color:#fff;border:0;border-radius:10px;padding:10px 14px;cursor:pointer}table{width:100%;border-collapse:collapse;margin-top:12px}" +
    "th,td{border-bottom:1px solid #eee;text-align:left;padding:8px}.muted{color:#666}.multi{min-height:120px}</style></head><body>" +
    ...
    "    <form id='f'>" +
    "      <div class='row'><div><label>Store ID</label><input name='store_id' value='" + (store_id || "") + "' required /></div>" +
    "      <div><label>Código del cupón</label><input name='code' placeholder='EJ: GIMNASIO10' required /></div></div>" +
    "      <label>Nombre interno</label><input name='name' placeholder='EJ: Convenio Gimnasios 10%' required />" +
    "      <div class='row'><div><label>Tipo de descuento</label><select name='discount_type'><option value='percent' selected>%</option><option value='fixed'>Monto fijo</option></select></div>" +
    "      <div><label>Valor</label><input name='discount_value' type='number' step='1' value='10' required /></div></div>" +
    "      <div class='row'><div><label>Vigencia desde</label><input name='valid_from' type='date' required /></div>" +
    "      <div><label>Vigencia hasta</label><input name='valid_until' type='date' required /></div></div>" +
    "      <div class='row3'><div><label>Ámbito</label><select name='apply_scope' id='apply_scope'><option value='all' selected>Toda la tienda</option><option value='categories'>Categorías incluidas</option><option value='products'>Productos (próximo)</option></select></div>" +
    "      <div><label>Mínimo carrito</label><input name='min_cart_amount' type='number' step='1' value='0' /></div>" +
    "      <div><label>Excluir productos en oferta</label><select name='exclude_sale_items'><option value='false' selected>No</option><option value='true'>Sí</option></select></div></div>" +
    "      <div id='cats_block' style='display:none'><label>Categorías para incluir (Ctrl/Cmd + clic para múltiples)</label><select id='include_categories' class='multi' multiple></select>" +
    "      <label style='margin-top:12px'>Categorías a excluir</label><select id='exclude_categories' class='multi' multiple></select></div>" +
    "      <div style='margin-top:12px;display:flex;gap:10px'>" +
    "        <button type='submit'>Crear campaña</button>" + // <-- SUBMIT REAL
    "        <button type='button' id='reload'>Actualizar lista</button>" +
    "      </div>" +
    "    </form><div id='msg' class='muted' style='margin-top:8px'></div></div>" +
    ...
    "function formToPayload(f){const fd=new FormData(f);const p={store_id:fd.get('store_id'),code:fd.get('code'),name:fd.get('name'),discount_type:fd.get('discount_type'),discount_value:Number(fd.get('discount_value')),valid_from:fd.get('valid_from'),valid_until:fd.get('valid_until'),apply_scope:fd.get('apply_scope'),min_cart_amount:Number(fd.get('min_cart_amount')||0),exclude_sale_items:toBool(fd.get('exclude_sale_items'))};if(p.apply_scope==='categories'){p.include_category_ids=selectedIds($('#include_categories'));p.exclude_category_ids=selectedIds($('#exclude_categories'));}return p;}" +
    ...
    "function renderList(rows){if(!rows||rows.length===0){$('#list').innerHTML='<p class=\"muted\">No hay campañas.</p>';return;}var html='<table><thead><tr><th>Nombre</th><th>Código</th><th>Tipo</th><th>Valor</th><th>Ámbito</th><th>Vigencia</th></tr></thead><tbody>';html+=rows.map(r=>{var val=r.discount_type==='percent'?(r.discount_value+'%'):('$'+r.discount_value);return '<tr><td>'+r.name+'</td><td><code>'+r.code+'</code></td><td>'+r.discount_type+'</td><td>'+val+'</td><td>'+r.apply_scope+'</td><td>'+r.valid_from+' → '+r.valid_until+'</td></tr>';}).join('');html+='</tbody></table>';$('#list').innerHTML=html;}" +
    "function refresh(){var sid=document.querySelector('input[name=store_id]').value.trim();if(!sid){$('#list').innerHTML='<p class=\"muted\">Ingresá Store ID arriba.</p>';return;}$('#list').textContent='Cargando…';listCampaigns(sid).then(renderList).catch(()=>{$('#list').innerHTML='<p class=\"muted\">Error cargando campañas.</p>';});}" +
    "function maybeLoadCats(){var scope=document.querySelector('#apply_scope').value;var block=document.querySelector('#cats_block');if(scope!=='categories'){block.style.display='none';return;}block.style.display='block';var sid=document.querySelector('input[name=store_id]').value.trim();if(!sid){document.querySelector('#msg').textContent='Ingresá Store ID para cargar categorías';return;}document.querySelector('#msg').textContent='Cargando categorías…';fetchCategories(sid).then(cats=>{var inc=document.querySelector('#include_categories');var exc=document.querySelector('#exclude_categories');inc.innerHTML=cats.map(c=>'<option value=\"'+c.id+'\">'+c.name+'</option>').join('');exc.innerHTML=cats.map(c=>'<option value=\"'+c.id+'\">'+c.name+'</option>').join('');document.querySelector('#msg').textContent='';}).catch(()=>{document.querySelector('#msg').textContent='No se pudieron cargar categorías';});}" +
    "document.querySelector('#apply_scope').addEventListener('change', maybeLoadCats);" +
    "document.querySelector('#f').addEventListener('submit', function(ev){ev.preventDefault();document.querySelector('#msg').textContent='Creando…';var payload=formToPayload(ev.target);api('/api/campaigns',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}).then(()=>{document.querySelector('#msg').textContent='Campaña creada ✅'; refresh();}).catch(e=>{document.querySelector('#msg').textContent='Error: '+(e.detail||e.message||'No se pudo crear');});});" +
